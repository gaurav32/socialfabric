...
GOOGLE_OAUTH_CLIENT_ID=$GOOGLE_OAUTH_CLIENT_ID GOOGLE_OAUTH_CLIENT_SECRET=$GOOGLE_OAUTH_CLIENT_SECRET GOOGLE_OAUTH_CALLBACK_URL=$GOOGLE_OAUTH_CALLBACK_URL PORT=3000 DATABASE_URL="postgres://postgres:postgres@localhost:5432/social_fabric" pnpm --filter @workspace/api-server run dev > cicd/server/deploy/logs/server.log 2>&1 &
